...
use crate::{layout::{Layout, LayoutId}, prelude::{Animatedf32, FillMode, FontId, InputState, Painter, Rect, Transform2D, Vec2, Vec4}};
...
impl<S: Signal> Widget for Collapse<S> {
	type Signal = S;
	...
	fn handle_event(&mut self, input_state: &mut InputState<Self::Signal>, id: LayoutId, area: Rect, _: Vec2) -> bool {
		...
		let should_switch = cursor_pos.iter().any(|pos| title_area.contains(*pos)) 
			&& self.signals.generate_signals(&mut self.inner, input_state, id, area, true, false).is_clicked;
		if should_switch {
			// println!("clicked");
