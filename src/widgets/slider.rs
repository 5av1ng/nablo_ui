...
pub struct Slider<S: Signal, A: App<Signal = S>> {
	...
	pub signals: SignalGenerator<S, SliderInner, A>,
...
impl<S: Signal, A: App<Signal = S>> Default for Slider<S, A> {
	fn default() -> Self {
		Self {
...
impl<S: Signal, A: App<Signal = S>> Slider<S, A> {
...
impl<S: Signal, A: App<Signal = S>> Widget for Slider<S, A> {
	type Signal = S;
	type Application = A;
