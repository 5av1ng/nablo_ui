...
impl<S: Signal> Widget for Divider<S> {
	type Signal = S;

	fn handle_event(&mut self, input_state: &mut InputState<S>, from: LayoutId, area: Rect, _: Vec2) -> bool {
		self.signals.generate_signals(&mut self.inner, input_state, from, area, false, false);
	...
	fn size(&self, id: LayoutId, painter: &Painter, layout: &Layout<S>) -> Vec2 {
